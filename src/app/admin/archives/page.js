...
export default function ArchivePage() {
  ...
  async function fetchArchivedItems() {
    setLoading(true);
    const [trucks, drivers, mechanics, inventory] = await Promise.all([
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {activeTab === 'trucks' ? 'Brand' : activeTab === 'inventory' ? 'Item Name' : 'Name'}
                    </th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Archived At
                    </th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data[activeTab].map((item) => (
                    <tr key={item.plate_number || item.driver_id || item.mechanic_id || item.item_id} 
                        className="hover:bg-gray-50 transition-colors duration-150">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-medium text-gray-900">
                          {item.plate_number || item.driver_id || item.mechanic_id || item.item_id}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-700">
                          {item.brand || item.name || item.item_name || '—'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
